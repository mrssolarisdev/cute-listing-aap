...
const ScreenShowList = ({route, navigation}) => {
    ...
    const [thisList, setThisList] = useState(route.params.list)
    ...
    useEffect(() => {
        if(editMode)
            navigation.setOptions({ headerTitle:
                <View>
                    <TextField onChange={(e) => handleEditedInput(e, true)} id="listTitle" defaultValue={listTitle} inputProps={{style:textInputStyles.title}}/>
                </View>
            })
        else {
            navigation.setOptions({ headerTitle: listTitle})
        }
    ...
    const handleBackBtn = () => {
        navigation.navigate("To Do", {listTitle: route.params.listTitle})
    }
